...
function pygments(code, lang, callback) {
  const codeStream = streamFromString(code);
  const child = child_process.spawn(
    "python",
    ["./src/processing/highlight.py", "--lang=" + lang],
    { stdio: ["pipe", "pipe", "pipe"] } // get stderr
  );
  ...
  child.on("close", code => {
    if (code !== 0) {
      callback(new Error(`Python script exited with code ${code}. Error: ${stderr}`), null);
    }
  });

  const outputStream = concatStream(data => {
    try {
  ...
  child.stdin.on("error", err => {
    if (err.code === 'EPIPE') {
      callback(new Error("Failed to write to Python stdin (EPIPE). Is the script crashing?"), null);
    } else {
      callback(err, null);
    }
  });
